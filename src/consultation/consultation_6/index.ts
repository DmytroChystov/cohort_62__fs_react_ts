...
// Благодаря типам редакторы (например, VS Code) лучше понимают ваш код
// , подсказывают методы, предупреждают об ошибках.
...
let petOfAlex2: ExtendedPet = "bird";
petOfAlex2 = "dog";
...
const users : User3[] = [
    {id: 1, name: "Vladimir", isAdmin: false },
    {id: 2, name: "Mikhail", isAdmin: true }
];
...
// nvm install node       # устанавливает последнюю версию Node.js
// nvm use node           # использовать установленную версию
...
// # Установка ts-node глобально (для запуска TS без компиляции)
// npm install -g ts-node
// ts-node -v             # проверка версии ts-node
